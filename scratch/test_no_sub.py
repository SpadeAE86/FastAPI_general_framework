...
def main():
    out_dir = r"./work/reproduce_no_sub"
    os.makedirs(out_dir, exist_ok=True)
    
    source_video = r"cache/0b/f5/a14205c09d01dd730d3a8a6ae709.mp4"
    segment_dur = 3.0

    # 1. GPU decoding (with -shortest but NO subtitle image)
    gpu_file = os.path.join(out_dir, "gpu_no_sub.mp4")
    cmd_gpu = [
    ...
    # 2. CPU decoding (with -shortest but NO subtitle image)
    cpu_file = os.path.join(out_dir, "cpu_no_sub.mp4")
    cmd_cpu = [
        "ffmpeg", "-y", "-ignore_editlist", "1",
    ...
    print("\n" + "="*60)
    print("NO-SUBTITLE ISOLATED DECODING COMPARISON")
    print("="*60)
    ...
    print(f"Loss: {cpu_info['duration'] - gpu_info['duration']:.3f}s ({(cpu_info['frames'] - gpu_info['frames'])} frames)")
    print("="*60)
