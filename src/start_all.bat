...
start "Celery Worker" cmd /k "call %CONDA_ACTIVATE% %CONDA_ENV% && cd /d %SRC_DIR% && python -m celery -A celery_mq.celery_app worker --hostname=local@%%h --loglevel=INFO --queues=local_video_queue --concurrency=1"
...
echo   - Celery Worker (local_video_queue)
echo.
echo 按任意键关闭此窗口...
pause >nul
