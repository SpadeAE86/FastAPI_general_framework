...
def load_config():
    ...
    if os.path.exists(config_file):
        ...
        # Get env from env, defaulting to config.yml value or "test"
        env_val = os.getenv("ENV", config_data.get("env", "test"))
        config_data["env"] = env_val
        
        # Get platform from env, defaulting to huawei
        platform = os.getenv("PLATFORM", "huawei")
        config_data["platform"] = platform
        
        # Copy settings from the selected platform to the root level
        platform_data = config_data.get(platform, {})
        for key in ["redis", "mysql", "rabbitmq", "object_storage"]:
            if key in platform_data:
                config_data[key] = platform_data[key]
