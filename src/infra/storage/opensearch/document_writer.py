...
# 公网 OpenSearch 上大包或长连接易被中间设备 RST；拆批 + 重试可显著降低失败率
_BULK_DOCS_PER_CHUNK = 20
_BULK_MAX_ATTEMPTS = 5
_BULK_RETRY_BASE_SEC = 1.0
_BULK_INTER_CHUNK_SLEEP_SEC = 0.08
# HTTP 读超时由 opensearch_connector 的 timeout 控制；bulk 侧靠拆批 + 重试应对 RST。
...
async def _bulk_single_request(
    ...
    *,
    ...
    for attempt in range(_BULK_MAX_ATTEMPTS):
        try:
            return await client.bulk(body=body, refresh=refresh)
        except BaseException as e:
            last = e
            if attempt + 1 >= _BULK_MAX_ATTEMPTS or not _is_retryable_bulk_error(e):
                log.error("bulk_index: giving up after %s attempts: %s", attempt + 1, e)
                raise
            wait = _BULK_RETRY_BASE_SEC * (2**attempt)
            log.warning(
                "bulk_index: attempt %s/%s failed (%s); retry in %.1fs",
                attempt + 1,
                _BULK_MAX_ATTEMPTS,
                e,
                wait,
...
async def bulk_index(
    ...
    *,
    ...
    Builds newline-style bulk body:
      {"index": {"_index": "...", "_id": "..."}}
      {...doc...}

    Large batches are split; transient connection errors are retried with backoff.
    """
    if not docs:
    ...
    if n <= _BULK_DOCS_PER_CHUNK:
        ...
        for i in range(0, n, _BULK_DOCS_PER_CHUNK):
            ...
            try:
                ...
                log.error(f"bulk_index failed at offset {i}: {e}")
                raise
            if last_resp.get("errors"):
                any_errors = True
                log.error(f"bulk_index errors in chunk @{i}: {last_resp}")
            if not is_last:
        ...
        if refresh and n > _BULK_DOCS_PER_CHUNK:
            try:
                ...
                log.warning("bulk_index: indices.refresh failed (non-fatal): %s", e)

    if last_resp.get("errors"):
        any_errors = True

    if any_errors:
        log.error(f"bulk_index errors: {last_resp}")
